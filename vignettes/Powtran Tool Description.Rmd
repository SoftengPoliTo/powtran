...
vignette: >
  %\VignetteIndexEntry{PowTrAn - Power Trace Analyzer}
...
-----------------  ----------  ----- ---------- --------
 Raspberry Pi 1A   Bubble      10k   `r round(mean(res$work$duration)*1000)`          `r format(length(power.samples)/1000,big.mark=",")`k
 
 LG Nexus 4        Quick       50k   `r round(mean(res.quick$work$duration,na.rm=T)*1000)`       `r format(length(power.samples2)/1000,big.mark=",")`k
-----------------  ----------  ----- ---------- --------
